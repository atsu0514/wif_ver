...
def main():
    ...
    # SVM 回帰モデル（RBFカーネル）を2出力用にラップ
    base_model = SVR(kernel="rbf", C=10.0, epsilon=0.1, gamma="scale")
    ...
    print("=== Match Rate (±10%以内, SVM, Test Only) ===")
    ...
    plt.plot(time_idx, heart_pred, label="Pred HeartRate (SVM, test)", alpha=0.7)
    ...
    plt.plot(time_idx, breath_pred, label="Pred BreathingRate (SVM, test)", alpha=0.7)
    plt.xlabel("Time index (relative)")
