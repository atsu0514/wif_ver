...
# cleaned_dataフォルダからCSVファイル一覧を取得 (globを使用)
search_pattern = os.path.join(DATA_DIR, "cleaned_sensor_data_*.csv")
CSV_LIST = glob.glob(search_pattern)
CSV_LIST.sort()  # 日付順などで処理されるようにソート

if not CSV_LIST:
    print(f"Warning: No files found in {DATA_DIR}")
else:
    print(f"Found {len(CSV_LIST)} CSV files in {DATA_DIR}")

# テスト用にするCSV (ファイル名のみ指定)
# cleaned_data内のファイル名に合わせて修正
TEST_CSV = "cleaned_sensor_data_20251203_232723.csv"
...
def main():
    ...
    # CSV_LIST はフルパスのリストになっているのでループ変数を path に変更
    for path in CSV_LIST:
        file_name = os.path.basename(path)
        
        if file_name == TEST_CSV:
            continue  # テストは混ぜない
        
        if not os.path.exists(path):
            print(f"警告: {path} が見つかりません。(スケーラー作成時) スキップします。")
            continue
    ...
    if not train_df_list:
        print("スケーラー作成用のTRAINデータがありません。cleaned_data フォルダを確認してください。")
        return
    ...
    for path in CSV_LIST:
        ...
        # --- 追加: データが空の場合はスキップ ---
        if len(df) == 0:
            print(f"警告: {file_name} はデータ行がありません。スキップします。")
            continue
        # --------------------------------------

        ds = SensorDataset(df, scaler_x, scaler_y, window_size=window_size)
        if len(ds) == 0:
            print(f"警告: {file_name} の有効サンプルが0です。スキップします。")
            continue

        if file_name == TEST_CSV:
            print(f"{file_name}: サンプル数 {len(ds)} → TEST 用として使用")
            test_datasets.append(ds)
        else:
            print(f"{file_name}: サンプル数 {len(ds)} → TRAIN 用として使用")
            train_datasets.append(ds)

    if not train_datasets:
        print("TRAIN 用データセットがありません。")
        return
    if not test_datasets:
        print(f"TEST 用データセットがありません。TEST_CSV={TEST_CSV} が存在するか確認してください。")
        return
