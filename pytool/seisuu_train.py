...
def main():
    ...
    print("Calculating thresholds based on Integer Rounding Error (Last Step)...")
    with torch.no_grad():
        for x_batch, y_batch in train_loader:
            # 推論 (Scaled)
            out = model(x_batch).cpu().numpy()     # (B, T, 2)
            y_batch_np = y_batch.cpu().numpy()     # (B, T, 2)
            
            # --- 逆変換 & 整数丸め ---
            b, t, features = out.shape
            
            # 2次元 (N, 2) にして一括逆変換
            out_reshaped = out.reshape(-1, features)
            y_reshaped = y_batch_np.reshape(-1, features)
            
            out_inv = scaler_y.inverse_transform(out_reshaped)
            y_inv = scaler_y.inverse_transform(y_reshaped)
            
            # 四捨五入して整数化
            out_int = np.round(out_inv)
            y_int = np.round(y_inv)
            
            # 元の形 (B, T, 2) に戻す
            out_int = out_int.reshape(b, t, features)
            y_int = y_int.reshape(b, t, features)
            
            # --- Last Step MSE (Integer Scale) ---
            # 最後の時刻のみ使用: (B, 2)
            last_pred = out_int[:, -1, :]
            last_true = y_int[:, -1, :]
            
            # 行ごとの平均二乗誤差 (B,)  -> (HeartDiff^2 + BreathDiff^2) / 2
            loss_batch = np.mean((last_pred - last_true) ** 2, axis=1)
            
            train_losses.extend(loss_batch)
    
    train_losses = np.array(train_losses)
    
    # 閾値を3段階で設定 (90%, 95%, 99%)
    threshold_low = np.percentile(train_losses, 87.0)
    threshold_mid = np.percentile(train_losses, 92.0)
    threshold_high = np.percentile(train_losses, 95.0)
    ...
    print(f"  Low (87%):  {threshold_low:.6f}")
    print(f"  Mid (95%):  {threshold_mid:.6f}")
    print(f"  High (92%): {threshold_high:.6f}")
