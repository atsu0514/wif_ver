...
def main():
    ...
    print("=== Match Rate (±10%以内, GRU, Multi-file, 指定TEST, GlobalScaler) ===")
    ...
    plt.plot(time_idx, heart_pred, label="Pred HeartRate (GRU, test)", alpha=0.7)
    ...
    plt.plot(time_idx, breath_pred, label="Pred BreathingRate (GRU, test)", alpha=0.7)
    plt.xlabel("Sample index (test file only)")
    ...
    plt.title("Anomaly Detection using GRU (Prediction Error on TEST_CSV)")
